...
int main() {
    ...
    std::vector<std::string> name{"EilerSys", "RK4Sys", "ErrorRK4", "ErrorEi"};
    ...
    RK4Sys<double>::answer(func, init,taurk4, t0, tn);
    ...
    std::fstream ErrorRk4;
    ErrorRk4.open("../ErrorRK4.txt",std::ios::out);
    tmp.open("../RK4Sys.txt",std::ios::in);
    for (int i=0; i<static_cast<int>((-t0 +tn) / taurk4);++i) {
        tmp>>t>>dx>>x;
        ErrorRk4<<t<< "\t"<<abs(dx+omega*std::sin(omega*t))<<"\t"<<abs(x-cos(omega*(t)))<<"\n";
    }
    tmp.close();
    ErrorRk4.close();

    std::fstream ErrorEi;
    ErrorEi.open("../ErrorEi.txt",std::ios::out);

    tmp.open("../EilerSys.txt",std::ios::in);
    for (int i=0; i<static_cast<int>((-t0 +tn) / tau0);++i) {
        tmp>>t>>dx>>x;
        ErrorEi<<t<< "\t"<<abs(dx+omega*std::sin(omega*t))<<"\t"<<abs(x-cos(omega*(t)))<<"\n";
    }
    tmp.close();
    ErrorEi.close();
    ...
    for (int i=0; i<name.size(); i++) {
        for (int j=2; j<=3;++j) {
            ...
            script << "set output '../Graphics/Task1/"+name[i]+'_'+var[j-2]+"(t).png'\n";
